...
class InsightsState(rx.State):
    ...
    def load(self):
        ...
        try:
            ...
            s=fetch_screener(self.symbol); n=fetch_nse(self.symbol); deals=fetch_nse_historical_deals(self.symbol,days=365)
            ...
            self.rows=[data]
            self.status=f"Loaded {self.symbol} · NSE institutional activity available"
        except Exception as e:self.status=f"Unable to load Insights: {e}"
